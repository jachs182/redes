...
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #indicates type of conection tcp in this case
s.bind((TCP_IP, TCP_PORT)) #asign the ip and the port for the socket
s.listen(1) #maximum of conections 
conn, addr = s.accept() #accepting conection from the client
...
def dwld():
    ...
    conn.send(bytes("you enter to dwld comand","utf-8"))
...
def delf():
    ...
    print("press 'Y' to confirm or any other key to abort")
    if confirm_delete == "Y":
        try:
            # Delete file
            os.remove(file_name)
